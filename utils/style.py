"""utils/style.py — Dark SOC theme CSS injection + session state bootstrap."""
...
.risk-bar-fill {
    animation: riskFill 0.8s ease-out;
...
def init_state() -> None:
    ...
    st.session_state.decision_eng  = DecisionEngine()
    st.session_state.node_engine   = NodeLocalizationEngine()
    st.session_state.nullifier     = AttackNullifier()
    st.session_state.state_machine = SystemStateMachine()
    st.session_state.forensic_tl   = ForensicTimeline()
    st.session_state.integrity_log = IntegrityLogger()
    # History buffers
    ...
    # Pipeline results
    st.session_state.pipeline_results = deque(maxlen=160)
    st.session_state.response_log     = []   # MitigationAction list
    st.session_state.detection_log    = []
    st.session_state.resp_log_dicts   = []
    ...
    st.session_state.attack_start_tick = 0
    st.session_state.evaluation_metrics = {}
    st.session_state.current_attack_type = "NONE"
    st.session_state.current_node_id     = "node_a"
