"""components/nodes.py — node status cards + geospatial map. Dark SOC theme.
All HTML via parenthesized f-strings (zero leading whitespace)."""
...
def render_node_status() -> None:
    ...
    for cel, (nid, lbl, lat, lng, color, mali) in zip(cols[:3], NODES):
        with cel:
            ...
            status = "BYZANTINE" if mali else "ONLINE"
            _html(
                f"<div style='background:linear-gradient(160deg,#0a0f1e 0%,#070c18 100%);"
                f"border:1px solid {color}22;border-left:3px solid {color};"
                f"border-radius:9px;padding:1rem;"
                f"font-family:{MONO};font-size:0.7rem;line-height:1.95;'>"
                f"<div style='display:flex;justify-content:space-between;"
                f"align-items:center;margin-bottom:0.4rem;'>"
                f"<span style='color:{color};font-weight:700;font-size:0.78rem;'>{nid}</span>"
                f"<span style='background:{bc};color:{sc};"
                f"font-size:0.57rem;padding:0.1rem 0.4rem;"
                f"border-radius:4px;letter-spacing:0.06em;'>{badge}</span>"
                f"</div>"
                f"<span style='color:#475569;'>{lbl}</span><br>"
                f"<span style='color:#1e293b;font-size:0.62rem;'>{lat:.4f} · {lng:.4f}</span><br>"
                f"<span style='color:{sc};'>● {status}</span><br>"
                f"<span style='color:#334155;'>{dp}</span><br>"
                f"<span style='color:{kc};'>KRUM: {kr}</span>"
                f"</div>"
            )

    with cols[3]:
        _html(
            f"<div style='background:linear-gradient(160deg,#0a0f1e,#070c18);"
            f"border:1px solid #0f172a;border-radius:9px;padding:1rem;"
            f"font-family:{MONO};font-size:0.68rem;line-height:2.1;'>"
            f"<span style='color:#3b82f6;'>L1</span> BATADAL 8761r<br>"
            f"<span style='color:#10b981;'>L2</span> IsolationForest<br>"
            f"<span style='color:#8b5cf6;'>L3</span> Flower FL 5 rounds<br>"
            f"<span style='color:#f59e0b;'>L4</span> epsilon-DP+Krum<br>"
            f"<span style='color:#ef4444;'>L5</span> Pearson w=20<br>"
            f"<span style='color:#06b6d4;'>L6</span> Streamlit live<br>"
            f"<span style='color:#1e293b;font-size:0.6rem;'>SDG 9·11·16·7</span>"
            f"</div>"
        )
...
def _folium_map() -> None:
    ...
    for nid, lbl, lat, lng, color, mali in NODES:
        popup_html = (
            ...
            f"padding:8px 10px;border-left:3px solid {color};min-width:160px;'>"
            f"<b style='color:{color};'>{nid}</b><br>"
            f"{lbl}<br>{lat:.4f} · {lng:.4f}<br>"
            f"{'BYZANTINE' if mali else 'ONLINE'} · KRUM: {'REJECTED' if mali else 'ACCEPTED'}"
            f"</div>"
        )
        ...
        folium.Marker(
            ...
            icon=folium.DivIcon(
                html=(
                    ...
                    f"color:{color};font-weight:700;white-space:nowrap;"
                    f"text-shadow:0 0 6px #000;'>{nid}</div>"
                ),
    ...
    stc.html(
        ...
        f"overflow:hidden;'>{map_html}</div>",
...
def _svg_map() -> None:
    ...
    stc.html(svg, height=340, scrolling=False)
